                                                         ...
                                                         size_t*  output_size )
{
    ...
    // Encode Header
    HIL_Application_Status_T tracker = HIL_APPLICATION_Header_Encoding( message, out_buffer );
...
HIL_APPLICATION_Decode_Message( const HIL_Application_Context_T* context,
                                const uint8_t* encoded_message, size_t max_encoded_message_size,
                                ...
                                size_t max_decoded_data_size, size_t* used_decoded_size )
{
    ...
    if ( max_encoded_message_size
         < HIL_APPLICATION_HEADER_SIZE_BYTES + 1 )  // +1 for the payload end flag
    {
        return HIL_APPLICATION_STATUS_BUFFER_TOO_SMALL;
    }
    ...
    HIL_Application_Status_T tracker =
        HIL_APPLICATION_Header_decoding( out_message, encoded_message, &expected_payload_size );
    if ( tracker != HIL_APPLICATION_STATUS_OK )
    {
        return tracker;
    };
