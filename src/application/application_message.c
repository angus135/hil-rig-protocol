                                                       ...
                                                       size_t* encoded_size )
{
     ...
     * variable pointer/count pairs, channel families, declared variable
     * lengths, and configured bounds. Calculate explicit future envelope/body
     * bytes using checked multiplication/addition/alignment, including the
     * fixed number of signal elements without serializing C padding. Do not use
     * native struct sizes as wire sizes, mutate or retain context/message, or
     * publish encoded_size before all validation and arithmetic succeeds.
     */
                                                         ...
                                                         size_t*  output_size )
{
     ...
     * record/span; reject inconsistent declarations; and publish output_size
     * only after complete success. Leave partial bytes unusable, retain no
     * pointer, and perform no reset, Transport behavior, semantic acceptance,
     * transaction mutation, test retention, execution-manager interaction, or
     * hardware behavior.
     */
    if ( output_size != NULL )
    {
                                     ...
                                     size_t* required_storage_size )
{
     ...
     * body fields, fixed tick-array lengths, variable counts/declarations, and
     * configured bounds; reject missing or trailing bytes; and calculate
     * aligned caller storage only for variable arrays/spans with checked
     * arithmetic. Fixed arrays decode inline and require no separate storage.
     * Publish required_storage_size only after complete success and neither
     * mutate nor retain context/input.
     */
    ...
    HIL_Application_Message_T* out_message, size_t* decode_storage_size )
{
     ...
     * test-ID presence; calculate/reserve aligned storage before publication;
     * decode explicit fixed-width fields in approved byte order; decode Global
     * Control and fixed signal arrays into the typed output; copy every variable
     * array/span into caller storage; point typed spans only there; reject
     * unsupported, truncated, malformed, inconsistent, or trailing data; and
     * publish out_message and storage-used atomically after complete success.
     * On small storage report the required size without partial typed output.
                                  ...
                                  const HIL_Application_Message_T* message )
{
     ...
     * element, variable pointer/count/zero-length
     * combinations, channel-family consistency, declaration/data lengths,
     * duty/range/count constraints, and
     * checked size arithmetic. Do not compare tick_number with an active Test
     * Configuration or enforce increasing upload order. Integration owns those
     * checks and transaction invalidation. Integration accepts each complete
     * tick only after taking responsibility for retaining its fixed and
     * declared variable data, automatically performs whole-test validation
     * after all N ticks and data arrive, and makes results available only after
     * execution completes. Do not evaluate retention medium/queue policy,
    ...
    size_t encoded_message_size, size_t* required_decode_storage )
{
     ...
     * enum/count/channel/declaration relationships, and configured bounds;
     * reject missing and trailing bytes; calculate variable decode storage
     * using checked arithmetic; and publish it only for a fully valid message.
