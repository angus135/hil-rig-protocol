...
constexpr std::array<HIL_Application_Message_Type_T, 5u> kPythonToFirmwareMessageTypes{
...
constexpr std::array<HIL_Application_Message_Type_T, 4u> kFirmwareToPythonMessageTypes{
...
std::array<HIL_Application_Message_T, 9u> ConstructEveryMessageFamily()
{
    ...
    messages[1].body.system_info_response.firmware_git_hash      = HIL_Application_Byte_Span_T{
        git_hash.data(), static_cast<std::uint8_t>( git_hash.size() ) };
    messages[1].body.system_info_response.diagnostic_data = HIL_Application_Byte_Span_T{
...
static_assert( kPythonToFirmwareMessageTypes.size() == 5u );
static_assert( kFirmwareToPythonMessageTypes.size() == 4u );
static_assert( kPythonToFirmwareMessageTypes.size() + kFirmwareToPythonMessageTypes.size() == 9u );
