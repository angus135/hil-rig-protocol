 ...
 * packed wire header. The codec explicitly serializes approved fields; native
 * structure layout is never the wire format.
 *
...
#define HIL_APPLICATION_VALID_TICK_PERIODS_US                                                      \
    {                                                                                              \
 ...
 * @details These constants define encoded bytes, not native C object sizes.
 * @{
 */
#define HIL_APPLICATION_PROTOCOL_MAJOR_SIZE_BYTES 1u
#define HIL_APPLICATION_PROTOCOL_MINOR_SIZE_BYTES 1u
#define HIL_APPLICATION_MESSAGE_HAS_ID_SIZE_BYTES 1u
#define HIL_APPLICATION_MESSAGE_TYPE_SIZE_BYTES 1u
#define HIL_APPLICATION_MESSAGE_SUB_TYPE_SIZE_BYTES 1u
#define HIL_APPLICATION_HEADER_PAYLOAD_SIZE_BYTES 2u
#define HIL_APPLICATION_HEADER_SIZE_BYTES                                                          \
    ( HIL_APPLICATION_PROTOCOL_MAJOR_SIZE_BYTES + HIL_APPLICATION_PROTOCOL_MINOR_SIZE_BYTES        \
 ...
 *
 * @details Numeric assignments are explicit stable one-byte wire identifiers.
 * Changing an assigned value after publication may break firmware/Python
 * compatibility and requires protocol-version review.
 ...
 * Configuration, Test Instruction, Update Instruction, Variable Instruction Data,
 * Execution Control, Test Result, Variable Result Data, and Variable Test Result,
 * optional for Error, and scope-dependent for Response. A Global Control Response
 * has no Test ID; all test-scoped Responses require one. test_id bytes are
 * ignored when has_test_id is zero; no byte pattern is reserved for absence.
 *
...
typedef struct
{
    ...
    /** Exactly one when test_id is present; exactly zero when it is absent. */
    uint8_t has_test_id;
    /** Opaque correlation identifier when has_test_id is one. */
